...
from dd.operators import LocalOperator, LocalModel
...
class LocalContext(Context):
    ...
    def __init__(self, db, uuid=None):
        Context.__init__(self, workflow=None, db=db, uuid=uuid)
        self.name = "Local"
        self.auto_persistence = True

    def build_dummy_operator(self, *args, **kwargs):
        """
        Dummy Operators cannot exist in a local context (it requires an instantiated DAG), so
        this function returns a local operator instead. The python callable for this operator
        should do nothing to match the Dummy Operator behaviour
        Args:
        ...
        Returns:
            A DummyOperator
        """
        return self.build_operator(*args, **kwargs)

    def build_operator(self, *args, **kwargs):
        """
        Returns an operator suited to this context (a LocalOperator).
        ...
        Returns:
            A LocalOperator
        """
        return LocalOperator(*args, **kwargs)

    def build_model_operator(self, *args, **kwargs):
        """
        Returns an operator suited to the context (a LocalModel).
    ...
    def _save_model(self, model, address):
        self._save_object(model, address)
