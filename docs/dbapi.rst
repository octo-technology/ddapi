...
Initially developed by yhat, the Data Driver version is customized to fit 
sepecific data science needs. For more informations, read the yhat `Blog Post <http://blog.yhathq.com/posts/introducing-db-py.html>`__.
...
``db.py`` is on `Anaconda <https://anaconda.org/octo/db.py>`__. The library is automatically installed with ddapi.

.. code:: bash

    $ conda install -c octo db.py 
